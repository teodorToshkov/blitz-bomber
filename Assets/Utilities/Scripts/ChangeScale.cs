...
public class ChangeScale : MonoBehaviour
{
    ...
    /// Sets the scale of the GameObject.
    /// </summary>
    /// <param name="scale">The scale in the form "X;Y;Z".</param>
    public void Set(string scale)
    {
        string[] buffer = scale.Split(';');
        Vector3 newScale;
        newScale.x = float.Parse(buffer[0]);
        newScale.y = float.Parse(buffer[1]);
        newScale.z = float.Parse(buffer[2]);
